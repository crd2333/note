...
- Non-Heightfield Terrain
  ...
  - Crazy Idea: Volumetric Representation
    - 王希老师说目前用的还不多（夹带私货），但据我所知现在 (2025) 已经有一些游戏用上了，比如《雾锁王国》基于体素的建造系统，比如 2025 年任天堂《大金刚》的体素破坏系统
...
- *Analytic Atmosphere Appearance Modeling*
  - 一个经验模型，定位类似于大气渲染中的 Blinn-Phong，给出方位角 $th, phi$ 就算出一个解析解得到颜色
...
- *Hi-Z*
  ...
  - 一些资料中说它是 CPU 端的剔除，又有一些资料中说它是 GPU-Driven Render Pipeline 的一环，个人理解其实两者都是对的
    ...
    - 如果是 GPU 端的剔除，涉及到 GPU -Driven Render Pipeline，修改风险太高且很多团队不具备该开发能力。当然现在随着 compute shader 的发展，在 GPU 上进行通用计算不再是新鲜事，慢慢地 Hi-Z 已经更多变成 GPU 端剔除的一环了
...
- 缺点
  ...
  + 透明物体处理困难（无法执行 blending，因为 G-Buffer 所有数据均来自单个片段）
...
- *Visibility Buffer*
  ...
  - 为什么要这么做呢？有下面几个原因
    ...
    + G-Buffer 希望将可见性与着色相分离，但实际上并不彻底！这将导致纹理读写层面的 overdraw，体现在两个方面
      + 如果没有做 Early-Z / Z-PrePass，最终被覆盖像素的纹理也要读纹理才能判断要不要写（而 Early-Z 在那些细碎树叶、透明物体等需要 alpha test 的场景是无法启用的）
        - 现代游戏中几何密度高到甚至超过像素，这个问题就更严重，在树叶、草繁多的场景尤为明显
      + 对每个光源的计算都是对 G-Buffer 本身作为纹理的一次采样，效率也非常低（除非把光源数量写死在一个 shader 里）
...
- 传统 CPU 驱动的渲染管线有何弊端？
  + draw primitives 比较昂贵，哪怕只画一个 triangle 也要把整个管线走一遍，为此经常需要 batch 化
    ...
    $ "Meshes" times "RenderStates" times "LoDs" times "Materials" times "Animations" $
  + GPU 等待 CPU 使其不能满载运行
    - CPU 不仅需要做 Frustum / Occlusion Culling，准备 drawcall，还要做各种模拟和逻辑运算，无法跟上 GPU
- 未来的趋势又如何？
...
- *Render to Monitor: V-Sync、G-Sync*
  - 显示器刷新频率是一定的，但 GPU 渲染频率会随场景复杂程度变化。如果显示器刷新时 GPU 正好写到一半，就会出现屏幕撕裂 (Screen Tearing) 的现象
