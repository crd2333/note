...
- 几个概念：
  ...
  + *Radiance, Irradiance and the Irradiance Volume*
    ...
    - 相关概念应用于 Irradiance Map, Radiance Map, Irradiance Volume 等，若不搞清楚，深入 GI 领域时将晕头转向。比如，笔者一度疑惑于为什么很多论文、算法中会有“Irradiance 的方向 $E(p, om)$”这一概念（明明我们这里说 $E(p)$ 是没有方向性的）
      ...
      - 单就上面这个问题，实际上就是对空间中任一点 $p$ 假设有一微表面，具有一个法向量。$E(p, om)$ 就是以这法向为中心对 $L(p, om)$ 做半球积分得到的 irradiance 值。这样的描述比原始 radiance 更低频（在方向上做了滤波）
    - 可以参考
      + #link("https://zhuanlan.zhihu.com/p/622940005")[四十九、The Irradiance Volume]，翻译、重述了 The Irradiance Volume 这篇经典论文（#link("https://www.sci.utah.edu/~bigler/images/msthesis/The%20irradiance%20volume.pdf")[原文 pdf]）
...
- *折射材质 (ideal reflective/refractive) 的 BRDF*
  - Snell's Law（斯涅耳定律，折射定律）：$$
    $
    n_1 sin theta_1 = n_2 sin theta_2 \
    cos theta_t = sqrt(1 - (n_1/n_2)^2 (1 - cos^2 theta_i))
    $
  - 有全反射现象（光密介质 $->$ 光疏介质）
  - 折射无法用严格意义上的 BRDF 描述，而应该用 BTDF (T: transmission)
    - 可以把二者统一看作 BSD F(S: scattering) = BRDF + BTDF
...
- 光子映射 (Photon Mapping)
  ...
  - 由于局部密度估计应该估计每个着色点的密度 $(dif N) / (dif A)$，但是实际计算的是 $(Delta N) / (Delta A)$，只有加大 $N$ 使 $Delta A$ 趋近于 $0$ 才能使估计值趋近于真实值，因此是一个有偏但一致的方法
...
- *次表面散射* (Subsurface Scattering)：光线从一个点进入材质，在表面的下方（内部）经过多次散射后，从其他一些点射出
  - 双向次表面散射反射分布函数 (BSSRDF)：是对 BRDF 概念的延伸，某个点出射的 Radiance 是其他点的入射 Radiance 贡献的
    #fig("/public/assets/CG/GAMES101/img-2024-08-04-21-35-28.png", width: 60%)
  - 计算比较复杂，因此又有一种近似的方法被提出 —— Dipole Approximation：引入两个点光源来近似达到次表面散射的效果
    #fig("/public/assets/CG/GAMES101/img-2024-08-04-21-38-45.png", width: 60%)
...
- 视场 (Field of Vied, FOV)
  - 定义针孔相机的 $h$ 和 $f$
    $ "FOV" = 2 * arctan(0.5 * h / f) $
  #fig("/public/assets/CG/GAMES101/img-2024-08-05-12-08-38.png",width: 50%)
...
- 薄透镜近似 (Thin Lens Approximation)
  ...
  - Circle of Confusion (CoC)：可以看出 $C$ 和 $A$ 成正比 —— 光圈越大越模糊
    #fig("/public/assets/CG/GAMES101/img-2024-08-05-13-53-49.png",width: 50%)
- 渲染中模拟透镜 (Ray Tracing Ideal Thin Lenses)
  ...
  - 好像还有简化的方法，参考 #link("https://blog.csdn.net/Motarookie/article/details/122998400#:~:text=简化实现方法")[这篇笔记]
- 景深 (Depth of Field)
...
== 全光函数与光场
- 全光函数是个 7D 函数，包含任意一点的位置 $(x, y, z)$、方向（极坐标 $th, phi$）、波长（描述颜色 $la$）、时间$(t)$
