...
- 从这张图中我们可以看到模型越来越趋向于整合，它的背后是任务的整合，把所有任务都表示成(vision plus text to text task)
  #fig("/public/assets/CV/Embodied/2024-10-06-13-11-04.png")
- #link("https://palm-e.github.io/")[PaLM-E]
  #fig("/public/assets/CV/Embodied/2024-10-07-11-11-52.png")
...
- VLM 大致架构（Google 内部使用的 PaLi），用 ViT 抽取图像特征，然后和文本信息一起送入另一个 Transformer
  #fig("/public/assets/CV/Embodied/2024-10-07-11-13-52.png", width: 70%)
- #link("https://robotics-transformer1.github.io/")[RT-1 架构]（时间：2022.12）
  #fig("/public/assets/CV/Embodied/2024-10-07-11-17-03.png")
...
- Represent actions in VLM
  ...
  #fig("/public/assets/CV/Embodied/2024-10-07-11-27-58.png", width: 80%)
- training the model
  #fig("/public/assets/CV/Embodied/2024-10-07-11-30-01.png", width: 80%)
  - 在预训练好的 VLM 上 co-fine-tune，即混合互联网图文数据和机器人数据，使得模型保持 Internet acale knowledge 并且不会因为过小的机器人数据而过拟合
    - 这可能是以后每个垂直领域都要用的方法
  - 机器人的数据基本上是构造成 QA 的形式
- Inference
  #fig("/public/assets/CV/Embodied/2024-10-07-11-29-10.png", width: 80%)
...
- 模型
  - 模型总览图
    #fig("/public/assets/CV/Embodied/2024-10-07-11-09-21.png")
  ...
  - 然后，原来的 LLM 可以视为 reward translation，基本上它分两阶段把自然语言 describe 成动作描述，再 encoder 成 reward functions
    ...
    #fig("/public/assets/CV/Embodied/2024-10-07-11-09-53.png")
- 实验结果
...
- 过去我们把语言模型视为一个语义引擎(semantic engine)，这里我们探索使用 LLM 的低级接口，本质上是要求它推理不同的序列，并且效果出奇地好，可以解决 ARC, PCFG 等任务，这里聚焦于 Sequence Improvement
  #fig("/public/assets/CV/Embodied/2024-10-07-11-07-45.png")
