...
- 很自然地，人体重建作为三维重建的子问题、子领域，3D Representation and Reconstruction 下的各种 Implicit and Explicit 方法都可以使用，无非是效果好坏的区别，如果从这种角度来分类的话：
  ...
  - *Explicit* 方面，用得最多的是 Voxel, Point Cloud, Mesh, 3DGS
    ...
    - *Mesh* 是人体重建这个领域用的最多的表示，大多数工作即使是用隐式表示，到最后都要把人体转化成 mesh
      ...
      - 另外也可能是受到 SMPL (mesh-based) 模型的影响。而且 SMPL 已经脱胎于普通的 mesh 表示，成为人体重建领域的基础 geometry prior，基于统计分布把人体变形分解为几组低维的参数化表示，大大缩小了合理的解空间，用来对抗单目数据的歧义性（反过来，如果不依赖于 SMPL，就要直接预测出那么多 vertices 的位置，而且还要满足正确拓扑关系，这是非常困难的）
- 不过如前所述，人体重建有几何、纹理颜色、高精度要求、训练数据稀缺等的复杂性
  - 这些复杂性导致这一领域的方法往往比较复杂，需要引入各种先验知识，包含很多 handcrafted 特征和模块（还没到能用大量数据进行端到端自监督训练的地步），各种方法的混合使用非常普遍（比一般三维重建领域更加hybrid），仍没有一个较好的范式来一统江湖
